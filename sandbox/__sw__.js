 ...
 * Version: 18 - multi-tab: map virtual ports to MessageChannels; cross-origin fix
 */

const DEBUG = false;

// Per-virtual-port MessageChannels (virtualPort -> MessagePort)
// Enables multiple tabs to run independent dev servers without collision.
const portChannels = new Map();

// Most recently initialized MessagePort (used to associate with server-registered)
let latestInitPort = null;
...
// Track which clients are in virtual contexts (clientId -> { port, prefix })
...
self.addEventListener('message', (event) => {
  ...
  if (type === 'init' && event.ports && event.ports[0]) {
    // Store as latest init port — will be associated with a virtual port
    // when the subsequent 'server-registered' message arrives.
    latestInitPort = event.ports[0];
    latestInitPort.onmessage = handleMainMessage;
  ...
  // Note: 'server-registered' and 'server-unregistered' arrive via the
  // MessageChannel (handled in handleMainMessage), not here.
});
...
function handleMainMessage(event) {
  ...
  // Handle server registration — associate virtual port with this MessagePort
  if (type === 'server-registered') {
    const virtualPort = data?.port;
    if (virtualPort && latestInitPort) {
      portChannels.set(virtualPort, latestInitPort);
      registeredPorts.add(virtualPort);
      DEBUG && console.log('[SW] Server registered on virtual port:', virtualPort, 'total channels:', portChannels.size);
    }
  }
  ...
  if (type === 'server-unregistered') {
    const virtualPort = data?.port;
    if (virtualPort) {
      portChannels.delete(virtualPort);
      registeredPorts.delete(virtualPort);
      DEBUG && console.log('[SW] Server unregistered from virtual port:', virtualPort, 'remaining channels:', portChannels.size);
    }
  }
}

/**
 * Get the MessagePort for a virtual server port, with recovery.
 */
async function getChannelForPort(port) {
  let msgPort = portChannels.get(port);
  if (msgPort) return msgPort;

  // No channel yet — ask all clients to re-initialize
  const allClients = await self.clients.matchAll({ type: 'window' });
  for (const client of allClients) {
    client.postMessage({ type: 'sw-needs-init' });
  }
  // Wait up to 5s for the channel to appear
  await new Promise(resolve => {
    const check = setInterval(() => {
      if (portChannels.has(port)) { clearInterval(check); resolve(); }
    }, 50);
    setTimeout(() => { clearInterval(check); resolve(); }, 5000);
  });

  msgPort = portChannels.get(port);
  if (!msgPort) {
    throw new Error(`No connection for virtual server on port ${port}`);
  }
  return msgPort;
}
...
async function sendRequest(port, method, url, headers, body) {
  DEBUG && console.log('[SW] sendRequest called, port:', port, 'url:', url);

  const msgPort = await getChannelForPort(port);
  const id = ++requestId;
...
self.addEventListener('fetch', (event) => {
  ...
  DEBUG && console.log('[SW] Fetch:', url.pathname, 'channels:', portChannels.size);
  ...
  if (!match) {
    ...
    // Strategy 1: Check referrer for virtual context
    ...
    // Strategy 2: Check if this client is tracked as being in a virtual context
    if (!virtualPort && event.clientId && virtualClients.has(event.clientId)) {
      const clientInfo = virtualClients.get(event.clientId);
      virtualPrefix = clientInfo.prefix;
      virtualPort = clientInfo.port;
    }

    if (virtualPort && url.origin === self.location.origin) {
      // Only intercept same-origin requests from virtual contexts.
      // Cross-origin requests (e.g. esm.sh CDN imports) must pass through directly.
      const targetPath = url.pathname + url.search;

      if (event.request.mode === 'navigate') {
        // Navigation requests: redirect to include the virtual prefix so the
        // browser's address bar and document.URL reflect the virtual path.
        ...
        // Non-navigation requests (scripts, CSS, images): forward to virtual server.
        // We must NOT redirect because that would change the module's registration URL
        // and cause ViteDevServer to inject /__virtual__/ into CDN import URLs.
  ...
  // Track this client as being in a virtual context (for requests without referrer)
  const clientInfo = { port, prefix: '/__virtual__/' + port };
  if (event.clientId) {
    virtualClients.set(event.clientId, clientInfo);
  }
  // For navigations, also track the resulting client (the new page being created)
  if (event.resultingClientId) {
    virtualClients.set(event.resultingClientId, clientInfo);
  }
